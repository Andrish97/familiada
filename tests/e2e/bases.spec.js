...
/* =====================================================================
   3) Mobile "sheet" modal (js/core/modal-sheet.js) -- na telefonie modal
   udostępniania/nazwy zastępuje treść strony zamiast być wyśrodkowanym
   okienkiem na przyciemnionym tle. Pierwsze użycie emulacji viewportu w
   tym repo -- patrz plan (docs, sekcja mobile sheet modals).
===================================================================== */
...
test.describe("bases: mobile sheet modal (udostępnianie/nazwa)", () => {
  ...
  test("modal udostępniania na telefonie zastępuje treść strony (sheet), nie jest małym okienkiem", async ({ page, context }) => {
    ...
    try {
      ...
      // 1) modal wypełnia szerokość viewportu, nie jest wyśrodkowanym boksem
      const box = await overlay.locator(".modal").boundingBox();
      expect(box.width).toBeGreaterThan(370);

      // 2) reszta treści strony (poza modalem i stopką) jest ukryta
      await expect(page.locator(".bar")).toBeHidden();

      // 3) stopka nadal obecna/widoczna
...
/* =====================================================================
   4) Regresja: confirmModal/alertModal (.uni-modal) MUSI zostać małym,
   wyśrodkowanym okienkiem na telefonie, nie dostać traktowania "sheet"
   (mechanizm jest wspólny dla całej strony -- wystarczy sprawdzić raz).
===================================================================== */
