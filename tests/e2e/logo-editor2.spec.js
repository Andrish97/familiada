...
test.describe("tryb Rysunek", () => {
  ...
  test("P0-3: stary rysunek (scena 700px) przeskalowany do stałego świata", async ({ page }) => {
    await open(page);
    const fabricData = {
      version: "5.3.0",
...
test.describe("telefon", () => {
  ...
  test("lista działa, edycja pokazuje komunikat zamiast edytora", async ({ page }) => {
    await open(page);
    const id = await L.insertLogo(page, { name: L.uniq("mobile"), type: "GLYPH_30x10", payload: L.textPayload() });
    await open(page);
