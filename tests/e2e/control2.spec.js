...
test("control2: pełna runda przez 4 urządzenia + wznowienie Control po przeładowaniu w środku rundy 2", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(page.getByText("Zgłoszono: Alfa")).toBeVisible({ timeout: 10000 });
    await page.getByRole("button", { name: "Przyjmij" }).click();

    // Odpowiedź #1 ma najwyższe punkty (40) — trafienie wygrywa pojedynek.
    await answerTile(page, 1).click();
    await answerTile(page, 2).click();
    await answerTile(page, 3).click();
...
test("control2: reset pojedynku, pass, kradzież wygrana/przegrana, odkrywanie reszty, koniec gry + Zakończ rozgrywkę", async ({ page, browser }) => {
  ...
  try {
    ...
    await answerTile(page, 1).click(); // A trafia (40 pkt) -> wygrywa pojedynek, BEZ nowego zgłoszenia
    ...
    await answerTile(page, 2).click(); // B kradnie WYGRANĄ (30 pkt)
    await expect(page.getByText("Bank: 70")).toBeVisible({ timeout: 10000 });

    await page.getByRole("button", { name: "Zakończ rundę" }).click();
    await answerTile(page, 3).click(); // #3 nieodkryte -> R8
    ...
    await expect(page.getByText("Zgłoszono: Beta")).toBeVisible({ timeout: 10000 });
    await page.getByRole("button", { name: "Przyjmij" }).click();

    await answerTile(page, 1).click(); // B trafia (40 pkt) -> kontrola B, allowPass
    await page.getByRole("button", { name: "Oddaj kontrolę" }).click(); // dawny "Pass" -> kontrola A

    await answerTile(page, 2).click(); // A trafia (30 pkt) -> bank 70
    await expect(page.getByText("Bank: 70")).toBeVisible({ timeout: 10000 });
    ...
    await page.getByRole("button", { name: "Zakończ rundę" }).click();
    await answerTile(page, 3).click(); // R8 ponownie
...
test("control2: próg w rundzie -> finał, wczesne zakończenie po 4/5 pytaniach, pomija gracza 2", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(page.getByText("Zgłoszono: Alfa")).toBeVisible({ timeout: 10000 });
    await page.getByRole("button", { name: "Przyjmij" }).click();
    await answerTile(page, 1).click(); // jedyna odpowiedź, 300 pkt -> bank 300
...
test("control2: physicalBuzzer + noHostTablet — urządzenia pominięte, ręczny wybór drużyny", async ({ page, browser }) => {
  ...
  try {
    ...
    await answerTile(page, 1).click(); // B trafia -> przejmuje kontrolę
    await expect(page.getByText("Bank: 40")).toBeVisible({ timeout: 10000 });
  } finally {
    for (const ctx of contexts) await ctx.close().catch(() => {});
    await deleteGame(page, game.id);
...
test("control2: \"Cofnij ostatnią akcję\" cofa ostatni zapis (3. pudło -> z powrotem 2.)", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(page.getByText("Zgłoszono: Alfa")).toBeVisible({ timeout: 10000 });
    await page.getByRole("button", { name: "Przyjmij" }).click();
    await answerTile(page, 1).click(); // A przejmuje kontrolę, bank 40
    ...
    await page.getByRole("button", { name: "X", exact: true }).click(); // xA=3 -> auto-STEAL
...
test("control2: wyścig — oba przyciski Buzzera naciśnięte w tej samej chwili, tylko jeden zaakceptowany", async ({ page, browser }) => {
  ...
  try {
    ...
    // "Zgłoszono:" pokazuje realną nazwę drużyny (Alfa/Beta), nie
    // wewnętrzny kod "A"/"B" — trzeba go odtworzyć z nazwy, żeby wskazać
    // właściwy #btnA/#btnB na stronie Buzzera.
    const zgloszono = page.getByText(/Zgłoszono: (Alfa|Beta)/);
    await expect(zgloszono).toBeVisible({ timeout: 10000 });
    const winner = (await zgloszono.textContent()).includes("Alfa") ? "A" : "B";
...
test("control2: wyciszenie dźwięku — po Mute żaden klucz SFX się nie odtwarza", async ({ page, browser }) => {
  ...
  try {
    ...
    await expect(page.getByText("Zgłoszono: Alfa")).toBeVisible({ timeout: 10000 });
    await page.getByRole("button", { name: "Przyjmij" }).click();
    await answerTile(page, 1).click(); // normalnie: buzzer_press + answer_correct
    await expect(page.getByText("Bank: 40")).toBeVisible({ timeout: 10000 });
