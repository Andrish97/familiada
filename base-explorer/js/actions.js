...
// ===== STATUS MAPS (folder badges / counts) =====
// NOTE: Must be module-scope (used by multiple actions).
...
export function wireActions({ state }) {
  ...
  function pulseDropTargetIn(containerEl, selector, fallbackEl) {
    ...
    if (el) return pulseEl(el);
    // fallback tylko jeśli to faktycznie wiersz (żeby nie mrugała cała lista)
    if (fallbackEl && (fallbackEl.classList?.contains("list-row") || fallbackEl.classList?.contains("row"))) {
      pulseEl(fallbackEl);
    }
  }
