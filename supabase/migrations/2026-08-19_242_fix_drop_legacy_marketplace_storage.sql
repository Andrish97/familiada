-- 242: Fix — retry of legacy marketplace/community-games cleanup
-- SUPERSEDES: 2026-08-19_241_drop_legacy_marketplace_storage.sql
--
-- 241 failed and rolled back entirely on its last step: this Supabase
-- instance has a trigger (storage.protect_delete()) that hard-blocks any
-- direct SQL DELETE on storage.buckets / storage.objects — "Use the
-- Storage API instead." So the bucket rows can NEVER be removed by a
-- migration; only via the Dashboard/Storage API (do that separately,
-- manually — see PR/chat notes).
--
-- This migration repeats everything from 241 EXCEPT the bucket DELETE.
...
-- 6. Drop RLS policies for the two dead buckets (harmless once the buckets are gone too)
...
-- NOTE: bucket rows themselves ('marketplace', 'community-games' in
-- storage.buckets) must be deleted manually via Supabase Dashboard ->
-- Storage -> Delete bucket. A SQL DELETE will always fail here
-- (storage.protect_delete() trigger) regardless of migration.
