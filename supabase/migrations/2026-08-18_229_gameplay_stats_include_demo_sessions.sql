-- Rozróżnienie: STWORZENIE gry demo nie jest działaniem użytkownika
-- (dostaje ją automatycznie), więc nadal pomijamy demo w licznikach
-- "gry"/"bazy pytań"/"logo". Ale ROZEGRANIE gry demo (np. testowanie
-- jak działa aplikacja) to już realne działanie użytkownika — więc
-- sekcja "gameplay" przestaje filtrować po is_demo. Jedyna zmiana w obu
-- funkcjach: usunięcie "AND g.is_demo = false" z zapytań o game_sessions.
-- Reszta (users/games/polls/bases/logos/mail/ratings) bez zmian.
