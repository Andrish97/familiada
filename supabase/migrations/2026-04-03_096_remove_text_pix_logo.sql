-- Remove logo_text_pix from demo_template_data and update functions
-- The text-pix mode has been removed from the UI; only text (GLYPH), draw, and image remain.

-- 1. Delete logo_text_pix demo entries
DELETE FROM demo_template_data WHERE slot = 'logo_text_pix';

-- 2. Update restore_my_demo function (remove logo_text_pix from logo slot list)
...
-- 3. Update seed_demo_for_user function (remove logo_text_pix from PIX logo array)
