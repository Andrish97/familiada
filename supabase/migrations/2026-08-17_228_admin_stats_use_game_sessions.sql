-- Podmiana źródła statystyk "gameplay" w panelu admina:
-- z device_presence (proxy "ekran był otwarty") na game_sessions_effective
-- (realne rozgrywki: start, przebieg, koniec, wynik, błędy, plus wpisy
-- 'legacy' z jednorazowego backfillu, żeby licznik "ile w ogóle" się zgadzał).
--
-- CREATE OR REPLACE z identyczną sygnaturą — bezpiecznie podmienia ciało,
-- nic innego się nie zmienia. Tylko sekcja "Gameplay" + klucz 'gameplay'
-- w wyniku JSON zostały dotknięte, reszta (users/games/polls/bases/logos/
-- mail/ratings) skopiowana 1:1 z obecnej wersji.
