-- 089: Final fix for schema state and queue system
-- SUPERSEDES: 2026-03-12_067_message_attachments.sql
-- SUPERSEDES: 2026-03-13_086_revert_to_586f3a9.sql
-- SUPERSEDES: 2026-03-13_087_queue_and_storage.sql
-- SUPERSEDES: 2026-03-13_088_fix_broken_state.sql
...
-- ============================================================================
-- 2. Game Generation Queue (Ensure Table and RLS)
-- ============================================================================
...
-- ============================================================================
-- 3. Market Games Schema (Safe migration)
-- ============================================================================
...
-- Safe data migration from gh_slug to storage_path
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'market_games' AND column_name = 'gh_slug') THEN
    UPDATE public.market_games 
    SET storage_path = 'admin/' || lang || '/' || gh_slug || '.json'
    WHERE storage_path IS NULL AND gh_slug IS NOT NULL;
    
    ALTER TABLE public.market_games DROP COLUMN gh_slug;
  END IF;
END $$;
...
-- ============================================================================
-- 4. RPC Functions (Ensure clean state)
-- ============================================================================

-- Drop all variants to avoid return type conflicts
