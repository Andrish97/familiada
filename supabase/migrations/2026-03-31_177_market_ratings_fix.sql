-- 177: Marketplace game ratings (retry after 176 failed — DROP functions first to allow return type change)
...
-- 7. market_browse — DROP first (return type changes), then recreate
...
-- 8. market_game_detail — DROP first, then recreate
...
-- 9. market_my_submissions — DROP first, then recreate
DROP FUNCTION IF EXISTS public.market_my_submissions();
